...
__global__ void SharedMemory_v3(float *C, const float *A, const float *B, int M, int K, int N)
{
    int tx = threadIdx.x; 
    int ty = threadIdx.y; 
    int bx = blockIdx.x;
...
PLAYGROUND_MATMUL_DEC(float32_t, 6, m, n, k, A, B, C)
{
    ...
    SharedMemory_v3<<<grid_dim, block_dim>>>(C, A, B, m, k, n);
