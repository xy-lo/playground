...
// Offset macro for 2D indexing
#define OFFSET(row, col, stride) ((row) * (stride) + (col))

using namespace nvcuda;

__global__ void __launch_bounds__(256, 2) wmmaDoubleBufferKernelv2 (
    ...
    size_t M, size_t N, size_t K)
{
    ...
    // WMMA fragments - reduced array size
    ...
    // Calculate loading indices for shared memory
    ...
    // Calculate which fragment this warp computes
    ...
    // Main loop with proper double buffering
    for (int tile = 0; tile < num_tiles; tile++) {
        // Select current and next buffer
        int smem_sel = tile & 1;
        int smem_sel_next = (tile + 1) & 1;
        ...
        // Load A fragments for first K-slice
        #pragma unroll
        for (int i = 0; i < 4; i++) {
        ...
        // Load B fragments for first K-slice
        #pragma unroll
        for (int j = 0; j < 4; j++) {
        ...
        // Load A fragments for second K-slice
        #pragma unroll
        for (int i = 0; i < 4; i++) {
        ...
        // Load B fragments for second K-slice
        #pragma unroll
        for (int j = 0; j < 4; j++) {
        ...
        // Perform matrix multiplication for both K-slices
        #pragma unroll
        for (int i = 0; i < 4; i++) {
            #pragma unroll
            for (int j = 0; j < 4; j++) {
...
PLAYGROUND_MATMUL_DEC(float16_t, 60, m, n, k, A, B, C)
{
    ...
    // Set maximum dynamic shared memory size
    cudaFuncSetAttribute(wmmaDoubleBufferKernelv2,
    ...
    wmmaDoubleBufferKernelv2<<<gridDim, blockDim, dsmem>>>(
